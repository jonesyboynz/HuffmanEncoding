...
TestSet* Huffman_tree_test_set(){
	...
	AddTest(set, NewTest("Tree frrequency is correct", Tree_frequency_is_correct_test, False));
...
bool Can_generate_tree_test(){
	...
	bool result = AssertEquals(heap->Count, 5);
...
bool Tree_structure_is_correct_test(){
	...
	result &= AssertEquals(heap->Heap[1]->Left->Left->Character, 'a');
	result &= AssertEquals(heap->Heap[1]->Left->Right->Character, 'b');
	result &= AssertEquals(heap->Heap[1]->Right->Character, 'c');
	DestroyHeapAndAllNodes(heap);
	return result;
}
...
bool Tree_frequency_is_correct_test(){
	...
	GenerateTreeWithinHeap(heap);
	bool result = AssertEquals(heap->Count, 1);
	result &= AssertEquals(heap->Heap[1]->Freq, 11);
	result &= AssertEquals(heap->Heap[1]->Left->Freq, 5);
	DestroyHeapAndAllNodes(heap);
	return result;
}
...
bool Tree_count_is_correct_test(){
	...
	HuffHeap* heap = GetHeapFromFrequencies(frequencies);
	GenerateTreeWithinHeap(heap);
	bool result = AssertEquals(Count(heap->Heap[1]), 5);
	result = AssertEquals(LeafCount(heap->Heap[1]), 3);
	DestroyHeapAndAllNodes(heap);
	return result;	
}